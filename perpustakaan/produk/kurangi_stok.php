    ...
    if ($jumlah <= 0) {
        ...
        if ($produk['stok'] < $jumlah) {
            ...
            try {
                ...
                $conn->commit();
                header("Location: ../barang_masuk_keluar.php?success=stock_reduced");
                exit();
            } catch (Exception $e) {
                $conn->rollback();
...
<head>
    ...
    <style>
        ...
        .alert-warning { background-color: #fff8e1; color: #ff8f00; border: 1px solid #ffcc02; }
    </style>
</head>
<body>
    <div class="container">
        ...
        <div class="product-info">
            ...
            <div class="stock-info">
                ...
                <div class="stock-box" id="resultBox">
                    <div>Stok Setelah Dikurangi</div>
        ...
        <form method="POST" id="kurangiStokForm">
            <div class="form-group">
                <label for="jumlah">Jumlah Stok yang Dikurangi</label>
                <input type="number" id="jumlah" name="jumlah" 
                       min="1" max="<?= $produk['stok'] ?>" value="1" required oninput="updateResult()">
                <small>Maksimal: <?= $produk['stok'] ?></small>
            </div>
            <div class="form-group">
                <label for="keterangan">Keterangan</label>
                <input type="text" id="keterangan" name="keterangan" placeholder="Contoh: Rusak, Hilang, atau digunakan" required>
            </div>
    ...
    function updateResult() {
        ...
        const hasil = Math.max(0, stokSaatIni - jumlah);
        ...
        if (hasil === 0) {
            ...
            resultValue.style.color = '#c62828';
        } else if (hasil < 10) {
    ...
    document.getElementById('kurangiStokForm').addEventListener('submit', function(e) {
        const jumlah = parseInt(document.getElementById('jumlah').value);
        const stokSaatIni = <?= $produk['stok'] ?>;
        const hasil = stokSaatIni - jumlah;
        
        if (hasil === 0) {
            if (!confirm('Peringatan! Stok akan menjadi 0. Apakah Anda yakin?')) {
                e.preventDefault();
            }
        } else if (hasil < 5) {
            if (!confirm('Peringatan! Stok akan sangat rendah (' + hasil + '). Apakah Anda yakin?')) {
                e.preventDefault();
            }
        }
    });
